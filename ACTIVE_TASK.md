...
## Status
In progress — validate and merge the HP big-ticket price-error fast lane.
...
## Validation required
- Compile the new service, app, configuration, and tests.
- Run targeted big-ticket qualification, protected-claim, scheduling, and configuration tests.
- Run import smoke and the complete pytest suite.
- Inspect review threads, conflicts, temporary files, and final diff before merge.

## Cleanup status
Pending CI and final diff inspection.

## Blockers
None.

## Backlog
- Deploy/redeploy the separate Discloud HP watcher after merge using the same Turso credentials as SniperPlug.
