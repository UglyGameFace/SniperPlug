...
class DealScannerCog(commands.Cog):
    ...
    async def _send_walmart_scan(self, interaction: discord.Interaction, query: str, min_discount: int, page: int, max_results: int, sort_value: str | None, order_value: str | None, alerts_only: bool, simple_mode: bool) -> None:
        ...
        if not cards:
            ...
            view = DealSearchControlView(query, page, max(0, shown_discount), max_results, sort_value, order_value, alerts_only, simple_mode)
            await interaction.followup.send(embed=summary, view=view, ephemeral=True)
            return

        embeds = [summary] + [card.embed for card in cards[:5]]
        view = DealSearchControlView(query, page, max(0, shown_discount), max_results, sort_value, order_value, alerts_only, simple_mode, cards[:5], result.has_next_page)
    ...
    async def walmart_scan_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError) -> None:
        ...
        await send_command_error(interaction, message)
...
class HuntPresetButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        key = ScanLockKey(interaction.guild_id, interaction.user.id, "hunt_preset", preset=self.preset.key, min_discount=self.preset.min_discount)
        if not await acquire_scan_lock(interaction, key, f"⏳ Running **{self.preset.label}**. Buttons are locked so this cannot double-post..."):
            return
        try:
            health_error = await provider_health_error_message()
            if health_error:
            ...
            await scan_operation_locks.release(key)
...
class DealSearchControlView(discord.ui.View):
    ...
    async def next_page(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._rerun(interaction, self.page + 1, self.min_discount)
    ...
    async def huge_discounts(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._hunt_pages(interaction, 80, 5)
    ...
    async def show_more(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        await self._rerun(interaction, self.page, max(0, self.min_discount - 20))

    async def _rerun(self, interaction: discord.Interaction, page: int, min_discount: int) -> None:
        key = ScanLockKey(interaction.guild_id, interaction.user.id, "deal_rerun", query=self.query, page=page, min_discount=min_discount)
        if not await acquire_scan_lock(interaction, key, f"⏳ Searching **{self.query}**. Buttons are locked so this cannot double-post..."):
            return
        try:
            ...
            if self.simple_mode:
                ...
                cards = build_walmart_cards(result, min_discount, self.alerts_only)
                shown_discount = min_discount
    ...
    async def _hunt_pages(self, interaction: discord.Interaction, min_discount: int, max_pages: int) -> None:
        key = ScanLockKey(interaction.guild_id, interaction.user.id, "hunt_pages", query=self.query, page=self.page, min_discount=min_discount)
        if not await acquire_scan_lock(interaction, key, f"⏳ Hunting **{min_discount}%+** for **{self.query}**. Buttons are locked so this cannot double-post..."):
            return
        try:
            ...
            for page in range(1, 1 + max_pages):
                ...
                aggregate = ProviderScanResult(result.provider_key, tuple(dedupe_candidates(all_candidates)), tuple(warnings), total_results, page, 25, 1, has_next_page)
                cards = build_walmart_cards(aggregate, min_discount, self.alerts_only)
                if cards:
                ...
                if not result.has_next_page:
                    break
            aggregate = ProviderScanResult("walmart", tuple(dedupe_candidates(all_candidates)), tuple(warnings), total_results, 1, len(all_candidates), 1, has_next_page)
...
async def acquire_scan_lock(interaction: discord.Interaction, key: ScanLockKey, working_message: str) -> bool:
    if not await scan_operation_locks.acquire(key):
        ...
        return False
    await mark_interaction_working(interaction, working_message)
    return True


async def mark_interaction_working(interaction: discord.Interaction, message: str) -> None:
    view = interaction.data and getattr(interaction, "message", None) and getattr(interaction.message, "components", None)
    disabled_view = disable_view(interaction.message.components) if view else None
    try:
        if not interaction.response.is_done():
            await interaction.response.edit_message(content=message, view=disabled_view)
        else:
...
def disable_view(_: object | None) -> discord.ui.View:
    view = discord.ui.View(timeout=300)
    button = discord.ui.Button(label="Working...", style=discord.ButtonStyle.secondary, disabled=True)
...
async def run_walmart_scan(query: str, page: int, max_results: int, sort_value: str | None, order_value: str | None, requested_by: str) -> ProviderScanResult:
    ...
    request = ProviderScanRequest(source_key="walmart", query=query.strip(), max_results=max_results, page=page, sort=sort_value, order=order_value, metadata={"requested_by": requested_by})
...
async def run_preset_hunt(preset: HuntPreset, requested_by: str) -> tuple[list[DealCard], int, int, list[str], int]:
    ...
    cards, shown_discount = cards_with_fallback(aggregate, preset.min_discount, False, fallback_chain)
    cards.sort(key=lambda card: (card.discount, card.score), reverse=True)
...
def cards_with_fallback(result: ProviderScanResult, requested_min_discount: int, alerts_only: bool, fallback_discounts: tuple[int, ...]) -> tuple[list[DealCard], int]:
    ...
    for threshold in thresholds:
        cards = build_walmart_cards(result, threshold, alerts_only)
        if cards:
            cards.sort(key=lambda card: (card.discount, card.score), reverse=True)
...
def build_walmart_cards(result: ProviderScanResult, min_discount: int, alerts_only: bool) -> list[DealCard]:
    ...
    for candidate in result.candidates:
        ...
        if alerts_only and not decision.should_alert:
            continue
        cards.append(DealCard(build_deal_card_embed(candidate, deal, decision, discount), deal.product_url, short_button_label(deal.title), decision.anomaly.score, discount))
    return cards
...
def build_deal_card_embed(candidate: SourceCandidate, deal: NormalizedDeal, decision, discount: float) -> discord.Embed:
    ...
    variant = variant_block(deal)
    if variant:
        embed.add_field(name="🎯 Selected option", value=variant, inline=False)
...
def variant_block(deal: NormalizedDeal) -> str:
    lines: list[str] = []
    if deal.variant_label:
    ...
    if deal.parent_title and deal.parent_title != deal.title:
        lines.append(f"Parent listing: {trim_title(deal.parent_title, 100)}")
    return "\n".join(lines[:8])
