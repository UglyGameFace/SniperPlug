...
HP_REFERENCE_SOURCES = {
    "hp.services.priceData.lPrice.msrp",
    "sniperplug.hp.exact_price_history.previous_price",
    "sniperplug.hp.exact_price_history.reference_price",
}


async def maybe_post_hp_deal_cards(
    *,
    bot: Any,
    guild_id: int | None,
    cards: list[Any],
    source_label: str = "hp_store_watcher:exact_verified",
    ...
    if db is None:
        return PublicPostResult(attempted=len(cards), errors=("HP public posting skipped: bot database unavailable",))
    ...
    try:
        ...
        return PublicPostResult(
            attempted=len(cards),
            errors=(f"HP category preference read failed; posting blocked: {clean_error_text(error)}",),
        )
    ...
    if channel is None:
        return PublicPostResult(attempted=len(cards), errors=(channel_note or "HP public channel lookup failed",))
    ...
    for card in cards:
        try:
            ...
            not_alertable += 1
            notes.append(f"HP category decision failed; card blocked: {clean_error_text(error)}")
            continue

        if not is_verified_hp_public_card(card, min_discount=min_public_discount):
            not_alertable += 1
            continue
        ...
        try:
            await mark_public_deal_sending(db, guild_id=int(guild_id), deal_key=deal_key)
            ...
            view = await build_deal_feedback_view(db, guild_id=int(guild_id), target=target)
            message = await channel.send(embed=sanitize_embed(card.embed), view=view)
        except Exception as error:
            await release_public_deal_reservation(db, guild_id=int(guild_id), deal_key=deal_key)
            notes.append(f"HP public post failed: {clean_error_text(error)}")
            continue
        ...
        if not finalized:
            notes.append("HP post sent but durable duplicate state could not be fully confirmed")
        try:
            await _cache_hp_active_deal(
                db,
                guild_id=int(guild_id),
                card=card,
                source_label=source_label,
            ...
            notes.append(f"HP active deal cache write failed: {clean_error_text(error)}")
        posted += 1
...
def is_verified_hp_public_card(card: Any, *, min_discount: int) -> bool:
    retailer = normalize_retailer_key(getattr(card, "retailer", None))
    if retailer != "hp":
        return False
    url = str(getattr(card, "direct_product_url", None) or getattr(card, "url", "") or "").strip()
    try:
        parsed = urlparse(url)
    except ValueError:
        return False
    if parsed.scheme != "https" or (parsed.hostname or "").lower() != "www.hp.com":
        return False
    if "/us-en/shop/pdp/" not in parsed.path.lower() and "/us-en/shop/products/" not in parsed.path.lower():
        return False
    ...
    if selected_offer != f"hp:{catalog_entry_id}:{sku}":
        return False
    if str(getattr(card, "seller_name", None) or "").strip().lower() != "hp.com":
        return False

    current = _positive_float(getattr(card, "api_current_price", None) or getattr(card, "current_price", None))
    reference = _positive_float(getattr(card, "api_reference_price", None) or getattr(card, "typical_price", None))
    reference_source = str(getattr(card, "api_reference_path", None) or attrs.get("trustedReferenceSource") or "")
    if current is None or reference is None or reference <= current:
        return False
    if reference_source not in HP_REFERENCE_SOURCES:
        return False
...
async def _cache_hp_active_deal(db: Any, *, guild_id: int, card: Any, source_label: str) -> None:
    await ensure_public_post_tables(db)
    conn = db.require_conn()
    from datetime import datetime, timezone

    now = datetime.now(timezone.utc).isoformat()
    active_key = card_product_key(card, retailer="hp")
    await conn.execute(
        """
        ...
        (
            guild_id,
            active_key,
            getattr(card, "label", None) or "HP deal",
            getattr(card, "url", ""),
