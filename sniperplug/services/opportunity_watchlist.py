from __future__ import annotations

from dataclasses import dataclass


@dataclass(frozen=True)
class OpportunityCategory:
    key: str
    label: str
    demand_level: int
    min_discount_percent: float
    absolute_price_floor: float | None
    terms: tuple[str, ...]
    economic_drivers: tuple[str, ...]


OPPORTUNITY_CATEGORIES: tuple[OpportunityCategory, ...] = (
    OpportunityCategory(
        key="gpus",
        label="Graphics Cards",
        demand_level=100,
        min_discount_percent=55,
        absolute_price_floor=300,
        terms=("rtx 5090", "rtx 5080", "rtx 5070", "rtx 4090", "gpu", "graphics card", "radeon rx"),
        economic_drivers=("ai demand", "gaming demand", "scarce supply", "component inflation"),
    ),
    OpportunityCategory(
        key="brand_direct_electronics",
        label="Brand-Direct Electronics",
        demand_level=96,
        min_discount_percent=45,
        absolute_price_floor=100,
        terms=("samsung", "lg oled", "sony bravia", "pixel", "galaxy", "surface", "xbox", "playstation", "oled tv", "qled", "monitor"),
        economic_drivers=("high resale value", "brand-direct coupon stacks", "trade-in pricing mistakes"),
    ),
    OpportunityCategory(
        key="sneakers",
        label="Sneakers / Streetwear",
        demand_level=94,
        min_discount_percent=45,
        absolute_price_floor=25,
        terms=("nike", "jordan", "air max", "dunk", "adidas", "yeezy", "puma", "new balance", "asics", "sneaker", "foot locker", "jd sports"),
        economic_drivers=("resale value", "drop demand", "member pricing", "checkout/coupon mistakes"),
    ),
    OpportunityCategory(
        key="cpus",
        label="CPUs",
        demand_level=92,
        min_discount_percent=50,
        absolute_price_floor=120,
        terms=("ryzen 9", "ryzen 7", "intel i9", "intel i7", "threadripper", "cpu", "processor"),
        economic_drivers=("ai demand", "pc upgrade cycle", "workstation demand"),
    ),
    OpportunityCategory(
        key="apple",
        label="Apple Products",
        demand_level=92,
        min_discount_percent=45,
        absolute_price_floor=100,
        terms=("iphone", "ipad", "macbook", "apple watch", "airpods", "mac mini"),
        economic_drivers=("high resale value", "strong consumer demand"),
    ),
    OpportunityCategory(
        key="ram",
        label="RAM / Memory",
        demand_level=95,
        min_discount_percent=45,
        absolute_price_floor=35,
        terms=("ddr5", "ddr4", "memory kit", "ram kit", "sodimm", "ecc memory"),
        economic_drivers=("memory shortage", "ai server demand", "component inflation"),
    ),
    OpportunityCategory(
        key="ssds",
        label="SSDs / Storage",
        demand_level=90,
        min_discount_percent=45,
        absolute_price_floor=30,
        terms=("nvme", "ssd", "m.2", "4tb", "8tb", "portable ssd", "external ssd"),
        economic_drivers=("nand price swings", "ai/server storage demand", "consumer storage demand"),
    ),
    OpportunityCategory(
        key="premium_apparel",
        label="Premium Apparel",
        demand_level=86,
        min_discount_percent=55,
        absolute_price_floor=20,
        terms=("lululemon", "north face", "patagonia", "carhartt", "arc'teryx", "columbia", "under armour", "sportswear", "hoodie", "jacket"),
        economic_drivers=("brand demand", "resale value", "seasonal clearance", "coupon stacking"),
    ),
    OpportunityCategory(
        key="gold_jewelry",
        label="Gold / Jewelry",
        demand_level=88,
        min_discount_percent=35,
        absolute_price_floor=50,
        terms=("10k", "14k", "18k", "gold chain", "gold bracelet", "gold coin", "gold bar", "diamond", "tennis chain", "zales", "kay jewelers"),
        economic_drivers=("gold price volatility", "inflation hedge", "resale value"),
    ),
    OpportunityCategory(
        key="watches",
        label="Watches",
        demand_level=82,
        min_discount_percent=45,
        absolute_price_floor=40,
        terms=("g-shock", "casio", "seiko", "citizen", "bulova", "fossil", "watch", "smartwatch"),
        economic_drivers=("resale value", "gift demand", "clearance cycles", "coupon stacking"),
    ),
    OpportunityCategory(
        key="fragrance_beauty",
        label="Fragrance / Beauty",
        demand_level=78,
        min_discount_percent=55,
        absolute_price_floor=15,
        terms=("cologne", "perfume", "fragrance", "eau de parfum", "eau de toilette", "dyson airwrap", "shark flexstyle"),
        economic_drivers=("gift demand", "brand demand", "coupon stacking", "holiday clearance"),
    ),
    OpportunityCategory(
        key="motor_oil",
        label="Motor Oil / Auto Fluids",
        demand_level=86,
        min_discount_percent=40,
        absolute_price_floor=8,
        terms=("motor oil", "synthetic oil", "5w-30", "0w-20", "diesel oil", "transmission fluid", "def fluid", "coolant"),
        economic_drivers=("oil price pressure", "vehicle maintenance inflation", "bulk household demand"),
    ),
    OpportunityCategory(
        key="tools",
        label="Tools",
        demand_level=84,
        min_discount_percent=50,
        absolute_price_floor=25,
        terms=("milwaukee", "dewalt", "makita", "ryobi", "tool set", "impact driver", "battery kit"),
        economic_drivers=("trade demand", "resale value", "home improvement demand"),
    ),
    OpportunityCategory(
        key="appliances",
        label="Appliances",
        demand_level=80,
        min_discount_percent=50,
        absolute_price_floor=75,
        terms=("washer", "dryer", "refrigerator", "freezer", "dishwasher", "microwave", "air conditioner"),
        economic_drivers=("housing costs", "home replacement demand", "open-box clearance"),
    ),
    OpportunityCategory(
        key="mobile_accessories",
        label="Mobile Accessories / Chargers",
        demand_level=98,
        min_discount_percent=35,
        absolute_price_floor=20,
        terms=(
            "wireless charging station",
            "3 in 1 charger",
            "3-in-1 charger",
            "magsafe",
            "magnetic charger",
            "phone charger",
            "iphone charger",
            "apple watch charger",
            "airpods charger",
            "charging dock",
            "charging stand",
            "charging station",
            "usb c charger",
            "power bank",
            "anker charger",
            "belkin charger",
            "phone case",
            "screen protector",
            "car phone mount",
        ),
        economic_drivers=("high household demand", "giftable tech", "apple accessory demand", "desk setup demand"),
    ),
    OpportunityCategory(
        key="viral_gadgets",
        label="Viral / Giftable Gadgets",
        demand_level=90,
        min_discount_percent=40,
        absolute_price_floor=25,
        terms=(
            "tiktok",
            "viral",
            "gadget",
            "led light",
            "sunset lamp",
            "mini printer",
            "portable blender",
            "ice maker",
            "neck fan",
            "handheld fan",
            "projector",
            "massage gun",
            "desk gadget",
        ),
        economic_drivers=("viral demand", "gift demand", "impulse-buy demand"),
    ),
    OpportunityCategory(
        key="smart_home",
        label="Smart Home / Security",
        demand_level=88,
        min_discount_percent=40,
        absolute_price_floor=30,
        terms=(
            "security camera",
            "doorbell camera",
            "smart lock",
            "smart plug",
            "smart bulb",
            "wifi camera",
            "robot vacuum",
            "dash cam",
            "ring camera",
            "blink camera",
            "wyze camera",
            "eufy camera",
        ),
        economic_drivers=("home security demand", "smart home upgrades", "bundle markdowns"),
    ),
    OpportunityCategory(
        key="office_school",
        label="Office / School / Desk Setup",
        demand_level=84,
        min_discount_percent=45,
        absolute_price_floor=20,
        terms=(
            "office chair",
            "desk",
            "standing desk",
            "printer",
            "label maker",
            "toner",
            "ink cartridge",
            "backpack",
            "school supplies",
            "desk organizer",
            "monitor arm",
            "laptop stand",
        ),
        economic_drivers=("work-from-home demand", "school season", "business purchasing"),
    ),
    OpportunityCategory(
        key="household_essentials",
        label="Household Essentials",
        demand_level=82,
        min_discount_percent=35,
        absolute_price_floor=15,
        terms=(
            "laundry detergent",
            "tide",
            "gain detergent",
            "paper towels",
            "toilet paper",
            "trash bags",
            "dish soap",
            "cleaning supplies",
            "swiffer",
            "lysol",
            "clorox",
            "soap",
            "body wash",
            "razor",
            "toothpaste",
        ),
        economic_drivers=("repeat household demand", "inflation pressure", "coupon stacking"),
    ),
    OpportunityCategory(
        key="baby_kids",
        label="Baby / Kids",
        demand_level=80,
        min_discount_percent=35,
        absolute_price_floor=20,
        terms=(
            "diaper",
            "baby wipes",
            "baby monitor",
            "stroller",
            "car seat",
            "booster seat",
            "crib",
            "baby formula",
            "toddler",
        ),
        economic_drivers=("family necessity", "high repeat demand", "registry/gift demand"),
    ),
    OpportunityCategory(
        key="pet_supplies",
        label="Pet Supplies",
        demand_level=78,
        min_discount_percent=35,
        absolute_price_floor=15,
        terms=(
            "dog food",
            "cat food",
            "cat litter",
            "pet bed",
            "dog crate",
            "pet carrier",
            "flea",
            "aquarium",
            "pet toy",
        ),
        economic_drivers=("repeat pet demand", "bulk buying", "subscription replacement demand"),
    ),
    OpportunityCategory(
        key="outdoor_sports",
        label="Outdoor / Sports",
        demand_level=82,
        min_discount_percent=45,
        absolute_price_floor=25,
        terms=(
            "bike",
            "electric scooter",
            "treadmill",
            "exercise bike",
            "weights",
            "dumbbell",
            "tent",
            "cooler",
            "grill",
            "fishing",
            "camping",
            "pool",
            "basketball hoop",
        ),
        economic_drivers=("seasonal demand", "fitness demand", "outdoor clearance"),
    ),
    OpportunityCategory(
        key="shoes_apparel",
        label="Shoes / Apparel",
        demand_level=84,
        min_discount_percent=50,
        absolute_price_floor=20,
        terms=(
            "nike",
            "jordan",
            "adidas",
            "puma",
            "new balance",
            "sneaker",
            "shoes",
            "boots",
            "hoodie",
            "jacket",
            "carhartt",
            "north face",
            "columbia",
            "under armour",
        ),
        economic_drivers=("brand demand", "seasonal clearance", "resale value"),
    ),
    OpportunityCategory(
        key="seasonal_holiday",
        label="Seasonal / Holiday",
        demand_level=76,
        min_discount_percent=50,
        absolute_price_floor=15,
        terms=(
            "christmas",
            "halloween",
            "easter",
            "valentine",
            "back to school",
            "pool clearance",
            "patio clearance",
            "holiday clearance",
            "seasonal clearance",
        ),
        economic_drivers=("seasonal liquidation", "clearance markdowns", "gift demand"),
    ),
    OpportunityCategory(
        key="walmart_cash",
        label="Walmart Cash Offers",
        demand_level=99,
        min_discount_percent=0,
        absolute_price_floor=None,
        terms=(
            "walmart cash",
            "walmart cash eligible",
            "cash offer",
            "cashback",
            "cash back",
            "onepay",
            "onepay cashrewards",
        ),
        economic_drivers=("extra Walmart Cash value", "cashback stacking", "hidden value beyond sticker price"),
    ),
    OpportunityCategory(
        key="home_kitchen",
        label="Home / Kitchen",
        demand_level=84,
        min_discount_percent=40,
        absolute_price_floor=20,
        terms=(
            "air fryer",
            "coffee maker",
            "keurig",
            "ninja",
            "vacuum",
            "shark vacuum",
            "bissell",
            "cookware",
            "mattress",
            "furniture",
            "patio furniture",
            "storage shelf",
            "humidifier",
            "heater",
            "fan",
        ),
        economic_drivers=("home demand", "gift demand", "seasonal clearance"),
    ),
    OpportunityCategory(
        key="grocery_pantry",
        label="Grocery / Pantry",
        demand_level=76,
        min_discount_percent=35,
        absolute_price_floor=10,
        terms=(
            "snacks",
            "coffee",
            "energy drink",
            "protein",
            "cereal",
            "soda",
            "water bottle",
            "pantry",
            "case pack",
            "grocery",
            "candy",
            "chips",
        ),
        economic_drivers=("repeat grocery demand", "bulk buying", "coupon stacking"),
    ),
    OpportunityCategory(
        key="toys_collectibles",
        label="Toys / Collectibles",
        demand_level=86,
        min_discount_percent=40,
        absolute_price_floor=20,
        terms=(
            "lego",
            "pokemon",
            "trading cards",
            "board game",
            "video game",
            "collectible",
            "hot wheels",
            "barbie",
            "nerf",
            "squishmallow",
            "toy clearance",
        ),
        economic_drivers=("gift demand", "collector demand", "holiday clearance", "resale potential"),
    ),
    OpportunityCategory(
        key="open_box_restored",
        label="Open Box / Restored / Refurbished",
        demand_level=88,
        min_discount_percent=35,
        absolute_price_floor=30,
        terms=(
            "restored",
            "refurbished",
            "open box",
            "like new",
            "restored: like new",
            "used",
            "excellent condition",
        ),
        economic_drivers=("condition-based markdowns", "electronics flips", "hidden variant pricing"),
    ),
    OpportunityCategory(
        key="health_wellness",
        label="Health / Wellness",
        demand_level=74,
        min_discount_percent=35,
        absolute_price_floor=15,
        terms=(
            "vitamin",
            "protein powder",
            "supplement",
            "fitness tracker",
            "blood pressure",
            "massage gun",
            "heating pad",
            "first aid",
            "personal care",
        ),
        economic_drivers=("repeat demand", "coupon stacking", "household need"),
    ),
    OpportunityCategory(
        key="business_bulk",
        label="Business / Bulk Supplies",
        demand_level=82,
        min_discount_percent=45,
        absolute_price_floor=20,
        terms=("amazon business", "business account", "bulk", "case pack", "quantity discount", "office chair", "toner", "printer paper"),
        economic_drivers=("business purchasing", "bulk pricing mistakes", "B2B coupon stacking"),
    ),
)


def category_for_title(title: str) -> OpportunityCategory | None:
    normalized = title.lower()
    matches = [category for category in OPPORTUNITY_CATEGORIES if any(term in normalized for term in category.terms)]
    if not matches:
        return None
    return sorted(matches, key=lambda category: category.demand_level, reverse=True)[0]


def high_demand_categories(limit: int = 10) -> list[OpportunityCategory]:
    return sorted(OPPORTUNITY_CATEGORIES, key=lambda category: category.demand_level, reverse=True)[:limit]
