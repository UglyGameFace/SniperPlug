...
def raw_price_signal(candidate: SourceCandidate, deal) -> bool:
    """Private raw-price review lead detector; does not create public deal proof."""
    ...
    return is_fragrance_or_beauty(title) and any(term in route for term in RAW_PRICE_ROUTE_TERMS)
...
def build_review_candidate_cards_with_raw_leads(candidates: Iterable[SourceCandidate], *, limit: int = 25) -> ReviewCandidateResult:
    ...
    for candidate in candidate_list:
        ...
        if deal.product_url in existing_urls or not raw_price_signal(candidate, deal):
            continue
        cards.append(_build_raw_price_card(candidate, deal))
