...
def eastern_tz() -> tzinfo:
    """Return America/New_York when tzdata exists, otherwise safe fixed ET.

    Termux and minimal containers may not include IANA tzdata. Importing this
    module must never crash bot startup because a local dev/runtime image lacks
...
def human_time(value: str | None) -> str:
    ...
    if not parsed:
        return "Not detected"
    return parsed.astimezone(eastern_tz()).strftime("%a %b %d, %I:%M %p ET")
