...
def install_strict_walmart_cash_guard() -> None:
    """Patch Walmart promo extraction so Walmart Cash never comes from generic reward text.

    Walmart's public docs do not expose a stable Affiliate/Product field contract for
    Walmart Cash. Until live payload paths are proven, SniperPlug only accepts values
    tied to explicit `Walmart Cash` / `walmartCash` evidence in the selected item
    payload and rejects ID/year/campaign-number style parses.
    """
    global _PATCHED
    if _PATCHED:
        return
