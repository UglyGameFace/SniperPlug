...
def is_marketplace_comp_source(source: str | None) -> bool:
    ...
    return (
        normalized in MARKETPLACE_COMP_SOURCES
        or "bestmarketplaceprice" in normalized
    )


def marketplace_comp_from_item(item: dict[str, Any]) -> dict[str, str]:
    """Extract alternate context and atomic selected-offer price truth.

    `bestMarketplacePrice` and top-level `minPrice` are alternate-offer
    context, never the selected buy-box price. Seller, offer ID, item price,
    same-offer reference, shipping, and delivered total are normalized from one
    selected-offer node so downstream code cannot combine different sellers.
    """
    attrs = selected_offer_delivery_attributes(item)

    best_marketplace = item.get("bestMarketplacePrice") or item.get(
        "best_marketplace_price"
    )
            ...
            attrs.update(
                {
                    ...
                    "marketplaceCompNote": (
                        "Walmart API marketplace comp; not selected offer or "
                        "was/regular price; use for flip research only"
                    ),
                }
            )
            seller = clean_string(
    ...
    if alternate_min is not None and alternate_min[1] > 0:
        ...
        attrs["alternateSellerMinPriceNote"] = (
            "alternate seller context only; never selected-offer price proof"
        )
...
def selected_offer_delivery_attributes(item: dict[str, Any]) -> dict[str, str]:
    ...
    if (
        marketplace is None
        and seller_name
        and not seller_name_is_walmart(seller_name)
    ):
        marketplace = True
    ...
    item_price = first_money(
        ...
        (
            f"{node_path}.priceInfo.currentPrice",
            nested(node, "priceInfo", "currentPrice"),
        ),
        ...
        (
            ...
            nested(item, "priceInfo", "currentPrice"),
        ),
        ("price", item.get("price")),
    )
    selected_reference = first_money(
        (f"{node_path}.wasPrice", node.get("wasPrice")),
        (
            f"{node_path}.priceInfo.wasPrice",
            nested(node, "priceInfo", "wasPrice"),
        ),
        (f"{node_path}.regularPrice", node.get("regularPrice")),
        (
            f"{node_path}.priceInfo.regularPrice",
            nested(node, "priceInfo", "regularPrice"),
        ),
        (f"{node_path}.strikeThroughPrice", node.get("strikeThroughPrice")),
        (f"{node_path}.strikethroughPrice", node.get("strikethroughPrice")),
        (f"{node_path}.originalPrice", node.get("originalPrice")),
        (
            f"{node_path}.priceInfo.originalPrice",
            nested(node, "priceInfo", "originalPrice"),
        ),
    )
    shipping = first_money_allow_zero(
        ...
        (
            f"{node_path}.priceInfo.shippingPrice",
            nested(node, "priceInfo", "shippingPrice"),
        ),
        (
            f"{node_path}.shippingOption.price",
            nested(node, "shippingOption", "price"),
        ),
        (
            f"{node_path}.shippingOption.cost",
            nested(node, "shippingOption", "cost"),
        ),
        ("shippingPrice", item.get("shippingPrice")),
        ("shippingCost", item.get("shippingCost")),
        ("shippingFee", item.get("shippingFee")),
        (
            "priceInfo.shippingPrice",
            nested(item, "priceInfo", "shippingPrice"),
        ),
    )
    ...
    if selected_reference is not None:
        reference_source, reference_price = selected_reference
        attrs["selectedOfferReferencePrice"] = f"{reference_price:.2f}"
        attrs["selectedOfferReferenceSource"] = reference_source
        attrs["selectedOfferReferenceTrusted"] = "yes"
    ...
    if item_price is not None:
        ...
        if shipping is not None:
            ...
            attrs["selectedOfferShippingStatus"] = (
                "free" if shipping_cost == 0 else "paid"
            )
            attrs["selectedOfferDeliveredPrice"] = (
                f"{price + shipping_cost:.2f}"
            )
            attrs["selectedOfferDeliveredPriceSource"] = (
                f"{price_source}+{shipping_source}"
            )
        elif explicit_free is True:
            attrs["selectedOfferShippingCost"] = "0.00"
            attrs["selectedOfferShippingSource"] = (
                "explicit freeShipping flag"
            )
            attrs["selectedOfferShippingStatus"] = "free"
            attrs["selectedOfferDeliveredPrice"] = f"{price:.2f}"
            attrs["selectedOfferDeliveredPriceSource"] = (
                f"{price_source}+freeShipping"
            )
        else:
            attrs["selectedOfferShippingStatus"] = "unknown"
            attrs["selectedOfferDeliveredPriceSource"] = (
                "blocked: shipping not returned"
            )

    return attrs


def selected_offer_node(
    item: dict[str, Any],
...
def marketplace_comp_from_attrs(
    attrs: dict[str, Any],
    ...
    if comp_price is not None:
        return (
            comp_price,
            comp_source or "Walmart API marketplace comp",
            comp_seller,
            comp_note,
        )
    ...
    return (
        context_price,
        context_source or "bestMarketplacePrice.price",
        None,
        "flip research only; not Walmart discount proof",
    )


def marketplace_api_lines(
    *,
    ...
    estimate = flip_estimate(
        walmart_price=current_price,
    ...
    if estimate:
        lines.append(
            ...
            f"spread **${estimate.spread:,.2f}** • "
            f"est. net **${estimate.net_estimate:,.2f}** • "
            f"ROI **{estimate.roi_percent:.1f}%** • {estimate.verdict}"
        )
        lines.append(
            "• Estimate assumptions: "
            f"{DEFAULT_MARKETPLACE_FEE_RATE:.0%} selling fee, "
            f"{DEFAULT_TAX_RATE:.0%} tax, "
...
def flip_estimate(
    *,
    ...
    if (
        walmart_price is None
        or walmart_price <= 0
        or comp_price is None
        or comp_price <= 0
    ):
        return None
