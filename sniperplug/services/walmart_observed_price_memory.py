...
class ObservedPriceMemorySelection:
    ...
    def summary_line(self) -> str:
        ...
        if self.cards:
            top = " | ".join(_short(card.label, 42) for card in self.cards[:3])
            parts.append(f"public price-drop cards: **{len(self.cards)}** ({top})")
        else:
            parts.append("public price-drop cards: **0**")
...
async def select_observed_price_drop_cards(
    ...
    *,
    ...
    """Record every exact Walmart candidate and return proven observed price drops.

    This lane is different from weak Walmart `was/list/MSRP` fields. It only uses
    SniperPlug's own prior API-observed current price for the same exact product
    identity, then posts when the current API price dropped enough.
    """
    ...
    for candidate in candidates:
        ...
        if decision.should_public_post and len(cards) < max(1, int(limit)):
            card = build_observed_price_drop_card(candidate, decision, min_discount=min_discount)
            cards.append(card)

    await conn.commit()
    cards.sort(key=lambda card: (float(getattr(card, "api_discount_percent", 0) or 0), float(getattr(card, "api_reference_price", 0) or 0)), reverse=True)
...
def build_observed_price_drop_card(candidate: SourceCandidate, decision: ObservedPriceMemoryDecision, *, min_discount: int) -> DealCard:
    ...
    title = _short(candidate.title, 82)
    embed = discord.Embed(
        title=f"📉 Walmart price drop • {title}",
        url=candidate.direct_product_url or candidate.product_url,
        description=(
            "SniperPlug is posting this because the **same exact Walmart item identity** is now lower than a prior API-observed price.\n"
            "This does **not** use MSRP/list/display text, marketplace comps, Walmart Cash, or search words as proof."
        ),
    ...
    embed.add_field(
        ...
        value=(
            f"Previous observed API price: **{money(reference)}**\n"
            f"Current API price: **{money(decision.current_price)}**\n"
            f"Observed drop: **{decision.drop_percent:.0f}%** / **{money(decision.drop_dollars)}**\n"
            f"Server threshold: **{int(min_discount)}%+**\n"
            f"Identity: `{decision.identity_key}`"
        ),
        inline=False,
    )
    status_bits: list[str] = []
    if candidate.stock_status:
        status_bits.append(f"Stock: **{_short(candidate.stock_status, 70)}**")
    if candidate.can_add_to_cart is True:
        status_bits.append("Add-to-cart: **observed**")
    if candidate.seller_name:
        status_bits.append(f"Seller: **{_short(candidate.seller_name, 70)}**")
    if candidate.fulfillment_type:
        status_bits.append(f"Fulfillment: **{_short(candidate.fulfillment_type, 70)}**")
    if status_bits:
        embed.add_field(name="📦 Buyability", value="\n".join(status_bits), inline=False)
    ...
    card = DealCard(
        ...
        api_reference_path="sniperplug.walmart_price_memory.previous_observed_api_price",
        api_price_path="walmart.api.current_price",
        ...
        variant_attributes={
            "priceMemoryIdentity": decision.identity_key,
...
async def upsert_candidate_memory(conn: Any, *, guild_id: int, identity_key: str, candidate: SourceCandidate, current_price: float | None, lowest_seen: float | None, now: str, status: str) -> None:
    ...
    await conn.execute(
        ...
        (
            ...
            candidate.sku,
            candidate.upc,
            candidate.selected_offer_id,
            current_price,
            next_lowest,
            candidate.seller_name,
            candidate.condition,
            candidate.fulfillment_type,
...
def is_candidate_buyable(candidate: SourceCandidate) -> bool:
    if candidate.option_mismatch_warning:
        return False
    if candidate.is_member_only or candidate.is_checkout_price or candidate.is_business_offer:
        return False
